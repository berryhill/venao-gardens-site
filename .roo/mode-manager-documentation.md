...
- **Key Features**:
  ...
  - Considers performance, security, and maintainability
   ...
   - Uses appropriate testing frameworks and mocking strategies
    - Uses DuckDuckGo Search MCP only for external PUBLIC resources explicitly listed in the issue
    - Uses Context7 MCP only for public external library documentation explicitly listed in the issue
    - Never attempts to use web/Context7 to access private repository docs or local files
    - Never uses ddg-search to access GitHub or repo content (private or public)
...
- **Key Features**:
   ...
   - Creates comprehensive documentation for components, APIs, features, and architecture
    - Uses codebase and docs as the primary source of truth for documentation
    - Reads local files directly for documentation context
    - Does NOT use Context7 for codebase-specific or private libraries
    - Does NOT use web searches to infer internal codebase behavior or APIs
    - Does not use the internet unless the orchestrator explicitly provides a public reference
    - Never uses ddg-search to access GitHub or repo content (private or public)

### tarxan-orchestrator
- **Purpose**: Coordinating complex, multi-step projects with documentation context passing
- **Key Features**:
  - Handles both frontend and backend tasks based on issue requirements
  ...
  - **Documentation Lookup**: Always examines existing documentation before delegating tasks (no delegation for doc lookup)
  ...
  - Prompts `tarxan-implementation-agent` to research public external resources using ddg-search MCP
  ...
  - May read the codebase and docs directly for context (local file reading is orchestrator-owned)
  ...
  - Only requires ddg-search research for public external resources explicitly listed in the issue
  - Never instructs ddg-search for GitHub or repo links (private or public)
...
- **Key Features**:
   ...
   - Uses the repository specified by the user's instructions or provided issue URL
   - Does NOT ask for confirmation when the user specifies the repo
...
## Mode Updates - 2026-01-20 (Orchestrator Scope and Access Fixes)

### tarxan-orchestrator
- Removed backend-only restriction; orchestrator now handles both frontend and backend tasks
- Clarified that the orchestrator may read the codebase and docs directly for context
- Updated workflow to delegate implementation → docs → optional platform tools, then PR
- Added instruction to re-delegate to implementation agent if PR CI/unit tests fail
- Added directive to never ask the user to confirm frontend vs backend
...
## Mode Updates - 2025-12-09 (Platform Tools Agent)

### tarxan-platform-tools-agent (NEW)
- **Purpose**: Maintaining Platform Agent tools, tests, and knowledge aligned with implementations
- **Key Features**:
  - Analyzes newly implemented features to identify needed Platform Agent tools
  - Creates new tools in `agent/platform_agent/tools/` following existing patterns
  - Writes comprehensive tests in `agent/platform_agent/tools/tests/`
  - Updates tool schemas following the existing format in the codebase
  - Updates the Platform Agent's system prompt when capabilities change
  - Creates/updates knowledge base documentation in `docs/PLATFORM_AGENT_*.md`
  - Ensures tools are properly registered and exported
  - Runs tests to verify tool functionality before completing
- **Workflow Position**: Triggered CONDITIONALLY after Implementation Agent and Documentation Agent
- **File Permissions**: Full edit access to all files
- **Branch Management**: Does NOT switch branches - orchestrator handles branch management

### Platform Feature Detection
The orchestrator now detects whether a feature requires Platform Agent tools:

**ENGAGE Platform Tools Agent for:**
- Pipelines (create, list, status, cancel, run)
- Workflows (list, details, execution)
- Agents (content generation, research, image generation)
- Integrations (social media publishing, OAuth connections, third-party services)
- Monitoring (health checks, metrics, activity tracking)
- Any feature that adds new operations users can perform on the platform

**DO NOT ENGAGE Platform Tools Agent for:**
- Authentication/Auth changes (login, logout, session management)
- UI/Style changes (CSS, layouts, visual updates)
- Infrastructure/DevOps changes (Docker, Kubernetes, CI/CD)
- Documentation-only changes
- Bug fixes that don't add new platform capabilities
- Refactoring that doesn't change functionality

### Updated Workflow Sequence
The orchestrator workflow now conditionally includes the Platform Tools Agent:
1. `tarxan-orchestrator` receives issue and creates feature branch
2. `tarxan-orchestrator` detects if this is a PLATFORM FEATURE
3. `tarxan-implementation-agent` implements core functionality AND unit tests on that branch
4. `tarxan-documentation-agent` updates documentation on that branch
5. **IF PLATFORM FEATURE**: `tarxan-platform-tools-agent` creates/updates Platform Agent tools and tests on that branch
6. `tarxan-orchestrator` creates pull request from that branch to dev

This addition ensures the Platform Agent stays aligned with new platform features, while avoiding unnecessary tool creation for non-platform changes.

## Mode Updates - 2026-02-03 (Implementation Agent Prompt Deduplication)

### tarxan-implementation-agent
- Removed duplicated header directives that repeated branch warnings at the top of the prompt.
- Removed redundant Git workflow and PR creation instructions that conflicted with orchestrator-only branch management.
- Kept core implementation, testing responsibilities, and MCP usage guidance intact.
